...
def load_and_process_data(file_path):
    ...
    proportions = data.groupby(['behav_data_ 1', 'behav_data_ 3']).apply(
        lambda x: np.mean(x['behav_data_10'] == x['behav_data_11'])
    ).reset_index(name='proportion')
    # Aggregate mean and std of proportions for each difficulty level
    mean_proportions = proportions.groupby('behav_data_ 3')['proportion'].mean()
    std_proportions = proportions.groupby('behav_data_ 3')['proportion'].std()
