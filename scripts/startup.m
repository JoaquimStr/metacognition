...
if server == 1
    basefolder_server = '/home/js2715/';
    toolboxesfolder = [basefolder_server, 'TOOLBOXES/'];
else
    basefolder_laptop = 'C:\';
    toolboxesfolder = [basefolder_laptop, 'TOOLBOXES\'];
end

% path definitions
ft_path = [toolboxesfolder, 'fieldtrip-20220310']; % Note Mac and Linux use forward slashes / instead of \
eeglab_path = [toolboxesfolder, 'eeglab2022.0']; 
adam_path = [toolboxesfolder, 'ADAM-master'];
% Hmeta_path = [toolboxesfolder, '/HMeta-d-master'];
...
% if (exist(Hmeta_path,'dir') == 7) && (~isdeployed)
%     addpath(genpath(Hmeta_path),'-begin');
%     disp('HMeta IS ALIVE');
% elseif ~isdeployed
%     disp(['WARNING, CANNOT FIND HMeta TOOLBOX AT ' HMeta_path ', CHECK PATHS IN startup.m']);
% end
