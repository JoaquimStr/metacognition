function first_level_as_Cate(params, response_type) 
...
if server == 1
    ...
    datafolder = [basefolder, 'rds/rds-tb419-bekinschtein/Joaquim/DATA (see Yanzhis folder)/READY/POST/CF/DROWSY'];
...
if strcmp(response_type, 'CF')
    ...
    cfg.outputdir = [resultfolder, '/CF/POST/AWAKE'];               % output location
    ...
    cfg.class_spec{1} = cond_string(low_confidence);                       % the first stimulus class
    cfg.class_spec{2} = cond_string(high_confidence);                      % the second stimulus class
    cfg.outputdir = [resultfolder, '/CF/POST/DROWSY'];              % output location
    adam_MVPA_firstlevel(cfg);                                             % run first level analysis
else
    ...
    cfg.outputdir = [resultfolder, '/PD/POST/AWAKE'];               % output location
    adam_MVPA_firstlevel(cfg);                                             % run first level analysis
