...
if server == 1
    ...
    datafolder = [basefolder, 'rds/rds-tb419-bekinschtein/Joaquim/DATA (see Yanzhis folder)/READY/REPLICATION_INDICES_CATE2/THREE/CD_SHORTEN'];
    scriptfolder = [basefolder, 'metacognition/'];
    resultfolder = [basefolder, 'rds/rds-tb419-bekinschtein/Joaquim/RESULTS/REPLICATION_INDICES_CATE2/TRIAL4'];
else
...
cfg.resample = 128;                                % downsample (useful for temporal generalization); see if 64Hz or 128Hz
...
cfg.erp_baseline = [-.25,0];                       % baseline correction in sec. ('no' for no correction) --> do we have pre-trial data for awake session?
...
%     cfg.filenames = drowsy_filenames;                                      % specifies filenames (awake in this case)
    ...
    cfg.outputdir = [resultfolder, 'CD'];                % output location
    cfg.save_confidence = 'yes';
    adam_MVPA_firstlevel(cfg);                                             % run first level analysis
