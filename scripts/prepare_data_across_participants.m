function prepare_data_across_participants(params, task_id)
...
if server == 1
    ...
    resultfolder = [basefolder, 'rds/rds-tb419-bekinschtein/Joaquim/DATA (see Yanzhis folder)/READY/across'];
...
save(strcat(subject, '_behav_ready_W_across.mat'), 'behav_data_w'); % save behavioural data after rejections and addition columns
save(strcat(subject, '_behav_ready_S_across.mat'), 'behav_data_s'); % for both awake and drowsy sessions
...
EEG = pop_saveset( EEG, 'filename', strcat(subject, '_clean_ready_W_across.set'),'filepath',strcat(halfway_datafolder)); % save a new dataset without those trials
...
if strcmp(subject, '108') 
    ...
    eeglab redraw; % redraw dataset
    EEG = pop_saveset( EEG, 'filename',strcat(subject, '_intermed_S_across.set'),'filepath',strcat(halfway_datafolder));
    cd(halfway_datafolder)
    eeg_file_s= strcat('108_intermed_S_across.set');
    EEG = pop_loadset(eeg_file_s); % load dataset in eeglab
end
...
% EEG_s = eeg_checkset(EEG);
eeglab redraw; % redraw dataset
EEG = pop_saveset( EEG, 'filename',strcat(subject, '_clean_ready_S_across.set'),'filepath',strcat(halfway_datafolder)); % save a new dataset without those trials
...
eeg_ready_w= strcat(subject, '_clean_ready_W_across.set'); % loading the new datasets 
eeg_ready_s= strcat(subject, '_clean_ready_S_across.set'); % for both awake and drowsy sessions
...
for epoch = 1:nb_events 
    for event = 1:length(EEG.event) % length of event fields
        if behav_data_w(epoch, 13) == 0 && behav_data_w(epoch, 11) == 0 && strcmp('SOUN', EEG.event(event).type) && EEG.event(event).epoch == epoch % we change all SOUN depending on confidence value from corresponding behavioural trial
            EEG.event(event).type = [num2str(task_id) '1']; % low confidence-tone event code = 11
        ...
        elseif behav_data_w(epoch, 13) == 0 && behav_data_w(epoch, 11) == 1 && strcmp('SOUN', EEG.event(event).type) && EEG.event(event).epoch == epoch % here for high confidence
            EEG.event(event).type = [num2str(task_id) '2']; % low confidence-cone event code = 12
        ...
        elseif behav_data_w(epoch, 13) == 1 && behav_data_w(epoch, 11) == 0 && strcmp('SOUN', EEG.event(event).type) && EEG.event(event).epoch == epoch % here for high confidence
            EEG.event(event).type = [num2str(task_id) '3']; % high confidence-tone event code = 13
        ...
        elseif behav_data_w(epoch, 13) == 1 && behav_data_w(epoch, 11) == 1 && strcmp('SOUN', EEG.event(event).type) && EEG.event(event).epoch == epoch % here for high confidence
            EEG.event(event).type = [num2str(task_id) '4']; % high confidence-cone event code = 14
...
% EEG_w = eeg_checkset(EEG);
eeglab redraw; % redraw dataset
EEG = pop_saveset( EEG, 'filename',strcat(subject, '_clean_ready_eventCodes_W_across.set'),'filepath',strcat(resultfolder)); % save dataset with confidence event codes
...
EEG_s = eeg_checkset(EEG);
eeglab redraw; % redraw dataset
EEG = pop_saveset( EEG, 'filename',strcat(subject, '_clean_ready_eventCodes_S_across.set'),'filepath',strcat(resultfolder)); % save dataset with confidence event codes
