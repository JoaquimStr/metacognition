function prepare_type2(params, state)
...
if server == 1
    ...
    resultfolder = [basefolder, 'rds/rds-tb419-bekinschtein/Joaquim/DATA (see Yanzhis folder)/READY/TYPE2/'];
    ...
    finalfolder = [resultfolder, 'CONFIDENCE ONLY/DROWSY'];
else
...
% 3) REJECT TRIALS TOO LOW RT AND OUTLIERS
behav_data = reject_RT(behav_data);

% 4) REJECT DIFFICULT TRIALS 
% behav_data = easy_only(behav_data);

% 5) RECODE CONFIDENCE
behav_data = recode_conf(behav_data, state);

% 6) REJECT TYPE II INCORRECT TRIALS!
behav_data = reject_incorrect_2(behav_data, state);
