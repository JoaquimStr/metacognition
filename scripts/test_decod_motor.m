function test_decod_motor(params)
%% This function prepares the data for the decoding of motor response (test)
...
if server == 1
    ...
    resultfolder = [basefolder, 'rds/rds-tb419-bekinschtein/Joaquim/DATA (see Yanzhis folder)/READY/motor/'];
    ...
    halfway_datafolder = [datafolder, 'WITHOUT_EVENTS_LP'];
    behav_results_folder = [datafolder, 'CLEAN_BEHAV_LP'];
    events_folder_W = [datafolder, 'EVENTS_COUNTER_LP\AWAKE'];
    events_folder_S = [datafolder, 'EVENTS_COUNTER_LP\DROWSY'];
end
...
position_w = 1;
for trial = 1:length(behav_data_w(:,1))
    if abs(behav_data_w(trial, 4)) < 0.3 % if lower than 0.3, considered as non-response (check)
        index_low_joystick_w(position_w) = trial;
        position_w = position_w + 1;
    end
end
...
position_s = 1;
for trial = 1:length(behav_data_s(:,1))
    if abs(behav_data_s(trial, 4)) < 0.3 % if lower than 0.3, considered as non-response (check)
        index_low_joystick_s(position_s) = trial;
        position_s = position_s + 1;
    end
end
...
save(strcat(subject, '_behav_ready_W_motor.mat'), 'behav_data_w'); % save behavioural data after rejections and addition columns
save(strcat(subject, '_behav_ready_S_motor.mat'), 'behav_data_s'); % for both awake and drowsy sessions
...
EEG = pop_saveset( EEG, 'filename', strcat(subject, '_clean_ready_W_motor.set'),'filepath',strcat(halfway_datafolder)); % save a new dataset without those trials
...
if strcmp(subject, '108') 
    ...
    eeglab redraw; % redraw dataset
    EEG = pop_saveset( EEG, 'filename',strcat(subject, '_intermed_S_motor.set'),'filepath',strcat(halfway_datafolder));
    cd(halfway_datafolder)
    eeg_file_s= strcat('108_intermed_S_motor.set');
    EEG = pop_loadset(eeg_file_s); % load dataset in eeglab
end
...
% EEG_s = eeg_checkset(EEG);
eeglab redraw; % redraw dataset
EEG = pop_saveset( EEG, 'filename',strcat(subject, '_clean_ready_S_motor.set'),'filepath',strcat(halfway_datafolder)); % save a new dataset without those trials
...
eeg_ready_w= strcat(subject, '_clean_ready_W_motor.set'); % loading the new datasets 
eeg_ready_s= strcat(subject, '_clean_ready_S_motor.set'); % for both awake and drowsy sessions
...
for epoch = 1:nb_events 
    for event = 1:length(EEG.event) % length of event fields
        if behav_data_w(epoch, 4) > 0 && strcmp('SOUN', EEG.event(event).type) && EEG.event(event).epoch == epoch % we change all SOUN depending on confidence value from corresponding behavioural trial
            EEG.event(event).type = '15'; % low confidence-tone event code = 11
            EEG.epoch(epoch).eventtype{1} = '15';
%             count_event_d(1) = count_event_d(1) + 1;
        elseif behav_data_w(epoch, 4) < 0 && strcmp('SOUN', EEG.event(event).type) && EEG.event(event).epoch == epoch % here for high confidence
            EEG.event(event).type = '16'; % low confidence-cone event code = 12
            EEG.epoch(epoch).eventtype{1} = '16';
...
% EEG_w = eeg_checkset(EEG);
eeglab redraw; % redraw dataset
EEG = pop_saveset( EEG, 'filename',strcat(subject, '_clean_ready_eventCodes_W_motor.set'),'filepath',strcat(resultfolder)); % save dataset with confidence event codes
...
for epoch = 1:nb_move 
    for event = 1:length(EEG.event) % length of event fields
        if behav_data_s(epoch, 4) > 0 && strcmp('SOUN', EEG.event(event).type) && EEG.event(event).epoch == epoch % we change all SOUN depending on confidence value from corresponding behavioural trial
            EEG.event(event).type = '15'; % low confidence-tone event code = 11
            EEG.epoch(epoch).eventtype{1} = '15';
            count_event_d(1) = count_event_d(1) + 1;
        elseif behav_data_s(epoch, 4) < 0 && strcmp('SOUN', EEG.event(event).type) && EEG.event(event).epoch == epoch % here for high confidence
            EEG.event(event).type = '16'; % low confidence-cone event code = 12
            EEG.epoch(epoch).eventtype{1} = '16';
            count_event_d(2) = count_event_d(2) + 1;
        end
...
EEG_s = eeg_checkset(EEG);
eeglab redraw; % redraw dataset
EEG = pop_saveset( EEG, 'filename',strcat(subject, '_clean_ready_eventCodes_S_motor.set'),'filepath',strcat(resultfolder)); % save dataset with confidence event codes
