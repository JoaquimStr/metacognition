function modify_events(behav_data, subject, state, finalfolder, halfway_datafolder) % count_event = 
...
EEG = pop_loadset([subject, '_step3_', state, '.set']); % load dataset in eeglab
...
pop_saveset( EEG, 'filename',strcat(subject, '_newPreprocessing_', state, '_2.set'),'filepath',strcat(finalfolder)); % save dataset with confidence event codes